...
void Alta()
{
	...
	for (int i = 0; i < alta; i++)
	{
		...
		printf("El n%cmero de item es %d %c\n \n", u,i + 1, c);
...
void listas()
{
	char c = 3,d=4, u = 163, z = 161, o = 162, e = 130;
	string filtro1, filtro2, filtro3;
	int opc;

	printf("\n %c %c %c MENU LISTA %c %c %c \n",c,c,c,c,c,c);
	printf("\n 1-Filtro g%cnero %c \n 2-Filtro clasificaci%cn %c\n 3-Filtro consola %c \n 4-Mostrar sin filtros %c \n",e,d,o,d,d,d);
	scanf_s("%d", &opc);
	switch (opc)
	{
	case 1:
		while (getchar() != '\n');
		printf("Ingrese el g%cenero que quiere buscar:\n",e);
		getline(cin, filtro1);
		for (int i = 0; i < alta; i++) 
		{
			if (genero[i] == filtro1 && stock[i] != "NO DISPONIBLE") 
			{
	...
	case 2:
		while (getchar() != '\n');
		printf("Ingrese la clasificaci%cn que quiere buscar:\n",o);
		getline(cin, filtro2);
		for (int i = 0; i < alta; i++)
		{
			if (clasif[i] == filtro2 && stock[i] != "NO DISPONIBLE")
			{
				printf("\n %c %c %c REGISTRO %d %c %c %c\n", c, c, c, i + 1, c, c, c);
				printf("El n%cmero del item es: %d \n", u, i + 1);
				printf("El nombre del articulo es: %s \n", nomart[i].c_str());
		...
		case 3:
			while (getchar() != '\n');
			printf("Ingrese la consola que quiere buscar:\n");
			getline(cin, filtro3);
			for (int i = 0; i < alta; i++)
			{
				if (consola[i] == filtro3 && stock[i] != "NO DISPONIBLE")
				{
					printf("\n %c %c %c REGISTRO %d %c %c %c\n", c, c, c, i + 1, c, c, c);
					printf("El n%cmero del item es: %d \n", u, i + 1);
					printf("El nombre del articulo es: %s \n", nomart[i].c_str());
...
void modificar()
{
	...
	printf("Ingrese que desea modificar: \n 1-Nombre del articulo %c\n 2-Descripci%cn %c\n 3-G%cnero %c\n", b, b, o, b, e, b);
	printf(" 4-Clasificaci%cn %c\n 5-Consola %c\n 6-Precio unitario %c\n 7-Stock %c\n", o, b, b, b, b);
	scanf_s("%d", &opcion);
